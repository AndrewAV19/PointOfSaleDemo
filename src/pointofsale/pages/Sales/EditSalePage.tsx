...
const EditSalePage: React.FC = () => {
  ...
  // Función para formatear la fecha y hora de la venta
  ...
  // Obtener fecha y hora formateadas de la venta
  const saleDateTime = formatSaleDateTime(selectedSale?.createdAt);
  ...
  const calculateTotalDiscount = () => {
    return productsList.reduce((acc, saleProduct) => {
      const originalTotal = saleProduct.quantity * saleProduct.product.price;
      const discountedPrice = saleProduct.product.discount
        ? saleProduct.product.price * (1 - saleProduct.product.discount / 100)
        : saleProduct.product.price;
      return acc + (originalTotal - discountedPrice * saleProduct.quantity);
    }, 0);
  };
  ...
  const handleConfirmEdit = async () => {
    ...
    try {
      ...
      const updatedSaleData: Partial<SaleRequest> = {
        client: client ? { id: Number(client) } : undefined,
  ...
  const handleDownloadTicket = async () => {
    if (ticketRef.current) {
      try {
        ...
        pdf.save(`ticket-venta-${selectedSale?.id || Date.now()}.pdf`);
                  ...
                  {productsList.length > 0 ? (
                    ...
                    >
                      <Table>
                        ...
                        <TableBody>
                          {productsList.map((saleProduct) => {
                            const discountedPrice = saleProduct.discount
                              ? saleProduct.product.price *
                                (1 - saleProduct.discount / 100)
                              : saleProduct.product.price;
                              ...
                              >
                                <TableCell>
                                  ...
                                  >
                                    ...
                                    <Box>
                                      ...
                                      {(saleProduct.discount ?? 0) > 0 && (
                                        <Chip
                                          label={`${saleProduct.discount}% OFF`}
                                          size="small"
                                          color="success"
                                          sx={{
                                            height: 20,
                                ...
                                <TableCell align="right">
                                  ...
                                  >
                                    $
                                    {(
                                      discountedPrice * saleProduct.quantity
                                    ).toFixed(2)}
                                  </Typography>
                                </TableCell>
                                <TableCell>
                    ...
                    >
                      ...
                      <Typography variant="body2" color="text.secondary">
                        Busca y agrega productos para editar la venta
                      </Typography>
                    </Paper>
                  )}
                </CardContent>
          ...
          >
            ...
            <List sx={{ flex: 1, overflow: "auto" }}>
              {productsList.map((saleProduct) => {
                ...
                return (
                  <ListItem key={saleProduct.product.id} divider sx={{ px: 0 }}>
                    <ListItemIcon>
                      <Avatar
                        src={saleProduct.product.image}
          ...
          <DialogContent>
            ...
            <Box ref={ticketRef} sx={{ p: 2, backgroundColor: "white" }}>
              ...
              <Box sx={{ textAlign: "center", mb: 2 }}>
                ...
                <Divider sx={{ my: 1 }} />
                <Typography variant="body2">
                  Venta #: {selectedSale?.id || "N/A"}
                </Typography>
                <Typography variant="body2">
                  Fecha: {saleDateTime.date}
                </Typography>
                <Typography variant="body2">
                  Hora: {saleDateTime.time}
                </Typography>
                <Divider sx={{ my: 1 }} />
              </Box>
              ...
              <Box sx={{ mb: 2 }}>
                ...
                {productsList.map((saleProduct, index) => {
                  const discountedPrice = saleProduct.discount
                    ? saleProduct.product.price *
                      (1 - saleProduct.discount / 100)
                    : saleProduct.product.price;
                  ...
                  return (
                    <Box key={saleProduct.product.id} sx={{ mb: 1 }}>
                      <Box
                        sx={{
                          display: "flex",
                          justifyContent: "space-between",
                      ...
                      >
                        <Typography variant="caption" color="text.secondary">
                          {saleProduct.quantity} x ${discountedPrice.toFixed(2)}
                        </Typography>
                        {saleProduct.discount && (
              ...
              {/* Información de pago si es pagada */}
              {saleStatus === "pagada" && (
                <>
                  <Box sx={{ mb: 1 }}>
                    <Box
              ...
              <Box sx={{ textAlign: "center", mt: 2 }}>
                <Typography variant="caption" display="block" gutterBottom>
                  Estado: {saleStatus.toUpperCase()}
                </Typography>
                <Typography variant="caption" display="block" gutterBottom>
                  ¡Gracias por su compra!
                </Typography>
                <Typography variant="caption" color="text.secondary">
                  {saleDateTime.date} {saleDateTime.time}
                </Typography>
              </Box>
            </Box>
          </DialogContent>
